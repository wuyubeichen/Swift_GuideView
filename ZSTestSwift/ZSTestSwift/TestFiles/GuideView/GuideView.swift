...
class GuideView: UIView {
    ...
    lazy var skipBtn:UIButton = {
       let skipBtn = UIButton(frame: CGRect(x: 0, y: 0, width: 0, height: 0))
        ...
        skipBtn.setTitle("立即体验", for: UIControlState.normal)
        skipBtn.setTitleColor(UIColor.white, for: UIControlState.normal)
        skipBtn.addTarget(self, action: #selector(skipBtnClicK), for: UIControlEvents.touchUpInside);
        return skipBtn
    }()
    
    
    //MARK: - Life Cycle
    init(images:Array<String>,viewType:GuidViewStyle) {
